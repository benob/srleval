...
class DependencyNode:
                ...
                for i in range(len(predicate_nodes)):
                    for node in nodes:
                        if node.argument_tokens[i] != "_":
                            ...
                            node.predicates.append((predicate_nodes[i], node.argument_tokens[i]))
            except:
    ...
    def isArgument(self, predicate, label=None):
        if label:
            return (predicate, label) in self.predicates
        return predicate in [x[0] for x in self.predicates]
